...
;;   clojure -Sdeps '{:paths ["src" "test" "resources"]}' -M scripts/live_loopback.clj $PIN
;;
...
(let [pin (or (first *command-line-args*)
  ...
  (try
    (let [res (client/handshake jdk/provider t {:server-name "localhost" :pin-spki-sha256 pin})]
      (if (r/error? res)
